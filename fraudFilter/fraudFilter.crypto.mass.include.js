...
async function main(data, attempt) {
    ...
    try {
        try {
            ...
            // await page2.goto(`${data.safePage}`, {
            //     waitUntil: "networkidle2",
            // });
            // const url = await page2.url();
            const url = data.safePage;
            data.safePage = url ? url : data.safePage;
        } catch (error) {
            console.log(error);
            console.log("broken --------- ", data.safePage);
        ...
        // await page.waitForTimeout(1000);
        // const blockCountry = await page.$("md-radio-button[aria-label='Block']"); // page
        // await blockCountry.click();

        // input aria-label="Select countries"
        const countries = data.country.split(";");
        for (let iCountry in countries) {
            const country_ = countries[iCountry];
            await page.waitForTimeout(1000);
            const inputCountry = await page.$("input[aria-label='Select countries']");
            await inputCountry.type(country_, { delay: 25 });
...
async function start() {
    const datas = require("./current.json");
