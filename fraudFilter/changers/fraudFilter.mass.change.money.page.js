...
async function main(data, attempt) {
    ...
    try {
        ...
        const tab = await page.waitForSelector("md-tab-item[aria-controls='tab-content-3']");
        await tab.click();
        // input name url ng-model="url.url"
        const tabContent3_1 = await page.$("md-tab-content[id='tab-content-3']");

        await page.waitForTimeout(1000);
        const inputMoney = await tabContent3_1.$("input[name='url']");
        await inputMoney.click({ clickCount: 3 });
        console.log(data.newLink);
        await inputMoney.type(data.newLink, { delay: 25 });
...
async function start() {
    const datas = require("./moneyChangeCurrent.json");
